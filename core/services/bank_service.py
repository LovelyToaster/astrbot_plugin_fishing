...
class BankService:
    ...
    def __init__(
        ...
        # 活期利率配置
        current_interest_rate: float = 0.0002,  # 活期基础利率，默认 0.02% / 小时 (7.3% 游戏年化)
        min_interest_rate: float = 0.00005,  # 最低活期利率，默认 0.005%
        max_interest_rate: float = 0.0005,  # 最高活期利率，默认 0.05%
        rate_volatility: float = 0.00003,  # 利率波动幅度，默认 0.003%
        # 定期利率配置
        base_fixed_rate: float = 0.0005,  # 基础定期利率，默认 0.05%
        max_fixed_rate: float = 0.08,  # 最高定期利率，默认 8.00%
        # 资金池影响参数
        ...
        # 定期利率配置（动态计算）
        # 新的定期利率采用动态计算：利率 = min(基础利率 + 天数奖励，最高 8.00%)
        # 使用单调递增公式，存期越长利率越高
        self.max_fixed_rate = max_fixed_rate  # 最高定期利率
        self.base_fixed_rate = base_fixed_rate  # 基础定期利率
        # 分段利率配置：(天数阈值，该段每增加 1 天的利率增量)
        # 15天: 0.60%, 30天: 1.50%, 60天: 3.60%, 120天: 8.00%
        self.tiered_rates = [
            (15, 0.00036667),  # 1-15 天：每天 +0.0367% (15天达到 0.60%)
            (30, 0.0006),      # 16-30 天：每天 +0.0600% (30天达到 1.50%)
            (60, 0.0007),      # 31-60 天：每天 +0.0700% (60天达到 3.60%)
            (120, 0.00073333), # 61-120 天：每天 +0.0733% (120天达到 8.00%)
        ]
